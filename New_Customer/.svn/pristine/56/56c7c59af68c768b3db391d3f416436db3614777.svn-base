		...
		<scroll-view scroll-y="true" :style="{ height: height }" style="padding: 10upx 0;" @scrolltolower="scrolltolower">
			<block v-for="(item, index) in dataList" :key="index">
				<view class="mdListBox">
					...
					<view class="map" @click="map(index)"><image mode="widthFix" src="../../static/tabbarImg/guide.png"></image></view>
				</view>
			</block>
		</scroll-view>
	</view>
...
export default {
	...
	methods: {
		...
		map(index) {
			let longitude = this.dataList[index].LongItude;
			let latitude = this.dataList[index].LatItude;
			// let StoreId = this.dataList[index].StoreId;
			let StoreName = this.dataList[index].StoreName;
			let address = this.dataList[index].Address;
